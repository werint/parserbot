...
SOURCE_SERVER_5_ID = 1084418816571879464  # Пятый сервер-источник
SOURCE_SERVER_6_ID = 1346249563270414469  # Шестой сервер-источник
SOURCE_SERVER_7_ID = 1150420551324672030  # Седьмой сервер-источник (ДОБАВЛЕН)
...
# Роли для проверки на пятом сервере
SOURCE_5_ROLE_IDS = [
    1084418816647368768,
    1084418816647368769
]

# Роли для проверки на шестом сервере
SOURCE_6_ROLE_IDS = [
    1462692338127077531
]

# Роли для проверки на седьмом сервере (ДОБАВЛЕНЫ)
SOURCE_7_ROLE_IDS = [
    1150422789778591764,
    1424882436294049873,
    1251618206905405573
...
TARGET_ROLE_5_ID = 1485994860962910289  # Пятая целевая роль (пятый сервер)
TARGET_ROLE_6_ID = 1491118136207085758  # Шестая целевая роль (шестой сервер)
TARGET_ROLE_7_ID = 1491909211263864833  # Седьмая целевая роль (седьмой сервер) (ДОБАВЛЕН)
...
class RoleSyncBot:
    ...
    async def check_user_roles(self, user_id):
        ...
        try:
                ...
                if source_member_6:
                    for role_id in SOURCE_6_ROLE_IDS:
                        ...
                        if role and role in source_member_6.roles:
                            ...
                            found_roles_sixth.append(f"{role.name} ({role.id})")
            
            # Проверяем седьмой сервер
            if source_server_7:
                ...
                if source_member_7:
                    for role_id in SOURCE_7_ROLE_IDS:
                        ...
                        if role and role in source_member_7.roles:
                            ...
                            found_roles_seventh.append(f"{role.name} ({role.id})")
            
            has_any_roles = has_first_server_roles or has_second_server_roles or has_third_server_roles or has_fourth_server_roles or has_fifth_server_roles or has_sixth_server_roles or has_seventh_server_roles
            ...
            return {
                ...
                'has_sixth_server': False,
                'has_seventh_server': False,
                'found_roles_first': [],
                'found_roles_second': [],
                'found_roles_third': [],
                'found_roles_fourth': [],
    ...
    async def check_and_sync_user(self, user_id, username=None, check_ban=True):
        ...
        try:
            ...
            if not target_role or not target_role_2 or not target_role_3 or not target_role_4 or not target_role_5 or not target_role_6 or not target_role_7:
            ...
            # Шестая роль (шестой сервер)
            if role_check['has_sixth_server'] and not has_target_role_6:
                try:
                    await target_member.add_roles(target_role_6, reason="Автоматическая синхронизация - шестой сервер")
                    actions_performed.append("✅ Выдана шестая роль")
                    print(f"✅ Выдана шестая роль пользователю {username} ({user_id})")
                except Exception as e:
                    print(f"❌ Ошибка при выдаче шестой роли: {e}")
            elif not role_check['has_sixth_server'] and has_target_role_6:
                try:
                    await target_member.remove_roles(target_role_6, reason="Автоматическая синхронизация - нет ролей на шестом сервере")
                    actions_performed.append("🗑️ Удалена шестая роль")
                    print(f"🗑️ Удалена шестая роль у пользователя {username} ({user_id})")
                except Exception as e:
                    print(f"❌ Ошибка при удалении шестой роли: {e}")
            
            # Седьмая роль (седьмой сервер)
            if role_check['has_seventh_server'] and not has_target_role_7:
                try:
                    await target_member.add_roles(target_role_7, reason="Автоматическая синхронизация - седьмой сервер")
                    actions_performed.append("✅ Выдана седьмая роль")
                    print(f"✅ Выдана седьмая роль пользователю {username} ({user_id})")
                except Exception as e:
                    print(f"❌ Ошибка при выдаче седьмой роли: {e}")
            elif not role_check['has_seventh_server'] and has_target_role_7:
                try:
                    await target_member.remove_roles(target_role_7, reason="Автоматическая синхронизация - нет ролей на седьмом сервере")
                    actions_performed.append("🗑️ Удалена седьмая роль")
                    print(f"🗑️ Удалена седьмая роль у пользователя {username} ({user_id})")
                except Exception as e:
                    print(f"❌ Ошибка при удалении седьмой роли: {e}")
            ...
            if actions_performed:
                log_msg = (
                    ...
                    f"• Шестой сервер: {'✅' if role_check['has_sixth_server'] else '❌'} {', '.join(role_check['found_roles_sixth']) if role_check['found_roles_sixth'] else 'Нет ролей'}\n"
                    f"• Седьмой сервер: {'✅' if role_check['has_seventh_server'] else '❌'} {', '.join(role_check['found_roles_seventh']) if role_check['found_roles_seventh'] else 'Нет ролей'}\n"
            ...
            if check_ban and not role_check['has_any_roles']:
                has_any_target_role = has_target_role or has_target_role_2 or has_target_role_3 or has_target_role_4 or has_target_role_5 or has_target_role_6 or has_target_role_7
...
async def on_ready():
    ...
    print(f'   Шестой сервер: {"✅" if source_server_6 else "❌"} {SOURCE_SERVER_6_ID}')
    print(f'   Седьмой сервер: {"✅" if source_server_7 else "❌"} {SOURCE_SERVER_7_ID}')
    ...
    startup_msg = (
        ...
        f"• Сервер 6: {'✅' if source_server_6 else '❌'} `{SOURCE_SERVER_6_ID}`\n"
        f"• Сервер 7: {'✅' if source_server_7 else '❌'} `{SOURCE_SERVER_7_ID}`\n"
...
async def check_user_command(ctx, user: discord.Member = None):
    ...
    try:
        ...
        report += f"• Сервер 6 ({SOURCE_SERVER_6_ID}): {'✅ Есть роли' if role_check['has_sixth_server'] else '❌ Нет ролей'}\n"
        
        if role_check['found_roles_sixth']:
            report += f"  Найденные роли: {', '.join(role_check['found_roles_sixth'])}\n"
        
        report += f"• Сервер 7 ({SOURCE_SERVER_7_ID}): {'✅ Есть роли' if role_check['has_seventh_server'] else '❌ Нет ролей'}\n"
        
        if role_check['found_roles_seventh']:
            report += f"  Найденные роли: {', '.join(role_check['found_roles_seventh'])}\n"
        ...
        report += f"• Роль 6 ({TARGET_ROLE_6_ID}): {'✅ Есть' if has_target_role_6 else '❌ Нет'}\n"
        report += f"• Роль 7 ({TARGET_ROLE_7_ID}): {'✅ Есть' if has_target_role_7 else '❌ Нет'}\n"
...
async def stats_command(ctx):
    ...
    with_role_6 = len([m for m in target_server.members if target_role_6 in m.roles]) if target_role_6 else 0
    with_role_7 = len([m for m in target_server.members if target_role_7 in m.roles]) if target_role_7 else 0
    ...
    stats_msg = (
        ...
        f"• С ролью 6: {with_role_6}\n"
        f"• С ролью 7: {with_role_7}\n"
...
def main():
    ...
    print(f"🔍 Серверов для проверки: 7")
